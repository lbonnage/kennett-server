...
if ps ax | grep -v grep | grep $SERVICE > /dev/null; then
	...
	then
		...
		if [ "$PLAYERSLIST" = "$PLAYERSEMPTY" ]
		then
			echo "Server is empty and shutdown will be initiated, first shutting down Minecraft server gracefully"
			$(screen -S minecraft -p 0 -X stuff "stop^M")
			while ps ax | grep -v grep | grep $SERVICE > /dev/null
			do
				echo "Server still shutting down, waiting for it to shut down gracefully"
				sleep 1m
			done
	...
	sleep 10m
	if ! ps ax | grep -v grep | grep $SERVICE > /dev/null; then
		echo "Screen does not exist, shutting down server"
		$(sudo /sbin/shutdown -P +1)
	fi
fi
